"""_gen_pearson_iqr_dist_S01_perclass.py — TEMP.

Pearson (0.5xIQR-cleaned, PER-CLASS) energy<->task correlation DISTRIBUTION
plots for S01 / MI / Sess01 / Orig / Fullband, over the 128 ICA sources, split
Artefact vs Non-Artefact. Generates BOTH 2-class and 3-class.

Two artefact definitions ("essais"):
  * AUTO   : artefact = offline EOG/EMG auto-detection (offline ICA).
  * MANUEL : artefact = every component NOT in the user-provided KEPT list
             (29 kept -> 99 artefact). Offline ICA is shared across 2-/3-class,
             so the same indices apply to both.

Outlier removal = PER-CLASS 0.5xIQR (Tukey fence within each finger class on
that source's energy), then signed Pearson(energy, one-vs-rest) over survivors.

Figures -> C:/Users/aymen/Desktop/S01 :
  DistPearson05IQR_S01_MI_Sess01_{n}class_1_Orig_Fullband_{finger}_{AUTO|MANUEL}.png
"""
import os, sys
...
SUBJ, SESS, MODEL = 1, 1, "Orig"
g.SUBJ = SUBJ
OUT = "C:/Users/aymen/Desktop/S01"
IQR_K = 0.5
NCLASSES = [2, 3]
FINGERS_BY_NCL = {2: ["Thumb", "Pinky"], 3: ["Thumb", "Index", "Pinky"]}

# User-provided KEPT (non-artefact) sources. MANUEL artefact = complement.
...
def main():
    ...
    print(f"MANUEL artefacts: {len(manual_art)} excluded / {len(kept)} kept",
          flush=True)
