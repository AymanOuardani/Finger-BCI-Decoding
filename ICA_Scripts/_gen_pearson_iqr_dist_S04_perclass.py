"""_gen_pearson_iqr_dist_S04_perclass.py — TEMP.

Pearson (0.5xIQR-cleaned) energy<->task correlation DISTRIBUTION plots for
S04 / MI / Sess05 / Orig / Fullband, over the 128 ICA sources, split
Artefact vs Non-Artefact. Generates BOTH 2-class and 3-class.

Outlier removal = PER-CLASS 0.5xIQR: for each source, drop trials outside the
[Q1 - 0.5*IQR, Q3 + 0.5*IQR] fence computed WITHIN each finger class on that
source's energy, THEN signed Pearson(energy, one-vs-rest membership) over the
survivors. (This reproduces the presentation tables; the older global-IQR
variant did not.)

Two artefact definitions ("essais") per condition:
  * AUTO      : artefact = offline EOG/EMG auto-detection (offline ICA).
  * CONFIRMED : artefact = the 103 manually-confirmed excluded components.

Figures -> C:/Users/aymen/Desktop/S09 :
  DistPearson05IQR_S04_MI_Sess05_{n}class_1_Orig_Fullband_{finger}_{AUTO|CONFIRMED}.png
"""
import os, sys
...
# 103 manually-confirmed excluded (= artefact) components, S04 Sess05 Orig.
# Offline ICA is shared across 2-class / 3-class, so the same indices apply.
CONFIRMED_EXCLUDED = [
...
# Sanity values (src80). Computed = per-class method; presentation = target table.
VALID_SRC = 80
VALID_PRES = {
    2: {"Thumb": +0.8630, "Pinky": -0.8630},
    3: {"Thumb": +0.8993, "Index": -0.4318, "Pinky": -0.4482},
}
...
def main():
    conf = set(CONFIRMED_EXCLUDED)
    assert len(conf) == 103, f"expected 103 confirmed-excluded, got {len(conf)}"
    art_auto = g.offline_artifacts()
    print(f"AUTO artefacts (offline EOG/EMG): {len(art_auto)}", flush=True)
    print(f"CONFIRMED artefacts: {len(conf)} excluded / {128 - len(conf)} kept",
          flush=True)

    splits = {
        "auto offline ICA  (EOG/EMG)":   ("AUTO",      art_auto),
        "103 confirmed-excluded comps":  ("CONFIRMED", conf),
    }
    ...
    for ncl in NCLASSES:
        ...
        # validation: per-class src80 vs presentation table
        keep80 = per_class_keep(E[:, VALID_SRC], classes)
        print(f"  validation src{VALID_SRC} (per-class 0.5xIQR, "
              f"kept {int(keep80.sum())}/{len(classes)}):", flush=True)
        for f in fingers:
            r = pearson(E[keep80, VALID_SRC], (classes[keep80] == f).astype(float))
            tgt = VALID_PRES[ncl][f]
            print(f"    {f:6s}: r={r:+.4f}  (presentation {tgt:+.4f}, "
                  f"diff={r - tgt:+.4f})", flush=True)
